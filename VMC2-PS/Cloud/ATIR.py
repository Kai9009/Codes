...
def Enc(PK, m, M, Att, SKpub, N=10, l=10, k=10):
    """[summary]#多行注释
    ...
    randomness = random.randint(0, len(Ci))
...
def AllEnc(PK, Att, N=10):
    ...
    m = Element(pairing, GT, value=pair ** r)
    l = k = 10
    M = GetMartix(params, l, k)
    CT = Enc(PK, m, M, Att, n, l, k)
    MainTimeEnd = datetime.now()
    ...
    # filesize_bytes = os.path.getsize(ParameterPathFromTools+"CT.dat")
    # logTime.info("%s is %s KB","CT.dat",filesize_bytes/1024)
    return CT


def Tag(PK, MSK, pub, w, SKpub):
    ...
    rix = Element.random(pairing, Zr)

    l = 10
    mu = {}
    for i in range(1, l + 1):
        mu[i] = Element.random(pairing, Zr)
    randomness = random.randint(0, len(mu))
    hash2_value = Element.from_hash(pairing, G1, Hash1(str(w).encode()).hexdigest())
    temp1 = Element(pairing, G1, value=K1pub * hash2_value)
    W1i = Element(pairing, G1, value=temp1 ** ri)
    W2i = Element(pairing, G1, value=K2pub ** ri)
    W3i = Element(pairing, G1, value=gal ** ri)
    W4i = Element(pairing, G1, value=g ** ri)
    temp2 = Element(pairing, G1, value=temp1 ** rix)
    T1i = Element(pairing, G1, value=g ** mu[randomness] * temp2)
    T2i = Element(pairing, G1, value=K2pub ** rix)
    T3i = Element(pairing, G1, value=gal ** rix)
    T4i = Element(pairing, G1, value=g ** rix)
    Wi = [W1i, W2i, W3i, W4i]
    Ti = [T1i, T2i, T3i, T4i]
...
def Trap(PK, w, SKsub):
    ...
    for j in range(1, lt + 1):
        ...
        Tdi[j] = lambt
    randomness = random.randint(0, len(Tdi))
...
def Policytest(PK, Wi, Tdsub):
    ...
    temp12 = Element(pairing, G1, value=temp1 * temp2)
    temp34 = Element(pairing, G1, value=temp3 * temp4)
    temp = Element(pairing, G1, value=temp12 / temp34)
...
def PreDec(PK, CT, Ti, Tdsub, pdksub):
    ...
    [T1i, T2i, T3i, T4i] = Ti
    [Td1j, Td2j, Td3j, Td4j] = Tdsub
    [K, K1sub, K2sub, K3sub, K4subq, Kchip, Ku] = pdksub
    pairing = Pairing(params)
...
def Trace(PK, SKsub, Att):
    ...
    temp5 = pairing.apply(K4sub, g ** (alpha + K))
    temp = pairing.apply(g, g)
    temp6 = Element(pairing, GT, value=temp ** alpha)
    temp7 = pairing.apply(K3sub ** K * K1sub, var)
    temp67 = Element(pairing, GT, value=temp6 * temp7)
...
def main(num):
    ...
    logger.info("==================skpub===============")
    print(SKpub)
    [SKsub, dksub] = KeyGensub(PK, MSK, ID, Att)
    logger.info("==================sksub===============")
    print(SKsub)
    print(dksub)
    CT = AllEnc(PK, Att, N=num)
    logger.info("=================CT===============")
    print(CT)
    [Wi, Ti] = Tag(PK, MSK, ID, w, SKpub)
    logger.info("================Wi===============")
    print(Wi)
    print(Ti)
    Tdsub = Trap(PK, w, SKsub)
    logger.info("================tdsub===============")
    print(Tdsub)
...
if __name__ == '__main__':
    MainTimeStart = datetime.now()
    logTime.info("!!!!!!!!!!!!!Now Attribute Num=10")
    main(10)
    logTime.info("!!!!!!!!!!!!!Now Attribute Num=20")
    main(20)
    logTime.info("!!!!!!!!!!!!!Now Attribute Num=30")
    main(30)
    logTime.info("!!!!!!!!!!!!!Now Attribute Num=40")
    main(40)
    logTime.info("!!!!!!!!!!!!!Now Attribute Num=50")
    main(50)
