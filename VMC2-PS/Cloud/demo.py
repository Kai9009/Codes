...
def Enc(PK,WindSet,M,PKOwners,n=10,ni=10,d=10,l=10):
    """[summary]
...
def AllEnc(PK,DList,PKOwners,n=10,ni=10):
    ...
    d=len(PKOwners)
    l=d
    M=GetMartix(params,d,l)

    AllC={}
    AllI={}
    for ind,WindSet in DList.items():
        [C,I]=Enc(PK,WindSet,M,PKOwners,n,ni,d,l)
        AllC[ind]=C
        AllI[ind]=I
    ...
    createFile(ParameterPathFromTools+"AllC.dat",str(AllC),"w")
    createFile(ParameterPathFromTools+"AllI.dat",str(AllI),"w")
    filesize_bytes = os.path.getsize(ParameterPathFromTools+"AllC.dat")
    logTime.info("%s is %s KB","AllC.dat",filesize_bytes/1024)
    filesize_bytes = os.path.getsize(ParameterPathFromTools+"AllI.dat")
    logTime.info("%s is %s KB","AllI.dat",filesize_bytes/1024)
    return [AllC,AllI]
...
def main(num):
    ...
    [AllC,AllI]=AllEnc(PK,DList,PKOwners,n=num,ni=num)
