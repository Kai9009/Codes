...
def Trace(PK,MSK,SKsub,Att):
    ...
    for i in range(1,len(Att)):
        att[i]=Element.random(pairing,Zr)
    randomness = random.randint(1, len(att))
    hash1_value = Element.from_hash(pairing, Zr, Hash1(str(att[randomness]).encode()).hexdigest())
