...
def KeyGenDU(PK, MSK, ID, Att):
    ...
    for i in range(1, n + 1):
        zDic[i]= Element.random(pairing, Zr)
    ...
    for i in range(1, n + 1):
        ...
        Ki1Dic = Element(pairing, G1, value=g ** (gamma + xij))
        Ki2Dic = Element(pairing, G1, value=g ** zDic[i])
...
def Enc(PK, DList, w, N):
    ...
    pi = {}
    for i in range(1,N):
        pi[i] = Element.random(pairing, Zr)
        for j in range(2,N):
            temp = Universe[i][j]
            Iij = Element(pairing, G1, value=temp ** pi[i])
    hash_value = Element.from_hash(pairing, Zr, Hash1(str(w).encode()).hexdigest())
    Iqq = Element(pairing, G1, value=beta ** (PI / hash_value))
    IiDic = Element(pairing, G1, value=g ** pi[i])
    IijDic = Element(pairing, G1, value=temp ** pi[i])
    I = [Iq, Iqq, IiDic, IijDic]
    ...
    # # createFile(ParameterPathFromTools + "Iw.dat", str(Iw), "w")
    # filesize_bytes = os.path.getsize(ParameterPathFromTools + "C.dat")
    # logTime.info("%s is %s KB", "C.dat", filesize_bytes / 1024)
    # logger.info("==================1=================")
    # # filesize_bytes = os.path.getsize(ParameterPathFromTools + "Iw.dat")
    # # logTime.info("%s is %s KB", "Iw.dat", filesize_bytes / 1024)
    return [C, I]
...
def Trap(PK, w, SKu, Att):
    ...
    for i in range(1,att):
        Ti1[i]= Element.random(pairing, Zr)
        Ti2[i] = Element.random(pairing, Zr)
    Ti1Dic = Element(pairing, G1, value=Ti1[i] ** mu)
    Ti2Dic = Element(pairing, G1, value=Ti2[i] ** mu)
...
def Search(PK, Att, Tw, C, I):
    ...
    Ti2Dic = Element(pairing, G1, value=str(Ti2Dic))
    att=len(Att)
    [Iq, Iqq, IiDic, IijDic] = I
    Iq = Element(pairing, GT, value=str(Iq))
    Iqq = Element(pairing, G1, value=str(Iqq))
    Ii={}
    for i in range(1, att):
        Ii = Element(pairing, G1, value=str(IiDic))
        Ti1 = Element(pairing, G1, value=str(Ti1Dic))
        Iij = Element(pairing, G1, value=str(IijDic))
        Ti2 = Element(pairing, G1, value=str(Ti2Dic))
    temp = pairing.apply(Ii, Ti1)
    ph1 = Element(pairing, GT, value=ph1 * temp)
    temp1 = pairing.apply(Iij, Ti2)
    ph2 = Element(pairing, GT, value=ph2 * temp1)

    #         Ii[i] = Element(pairing, G1, value=str(IiDic))
    # randomness = random.randint(1, len(Ii))



    # Ii = Element(pairing, G1, value=str(Ii[randomness]))
    # Ti1 = Element(pairing, G1, value=str(Ii[randomness]))
    # temp = pairing.apply(Ii, Ti1)
    # ph1 = Element(pairing, GT, value=ph1 * temp)
    #
    # Iij = Element(pairing, G1, value=str(Ii[randomness]))
    # Ti2 = Element(pairing, G1, value=str(Ii[randomness]))
